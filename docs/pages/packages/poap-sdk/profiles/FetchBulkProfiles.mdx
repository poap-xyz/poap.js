...
A `Map` where keys are the queries that failed and values are `ProfileError` objects. This is for actual processing errors, such as a failure to resolve data from the blockchain, not for profiles that are simply not found.
