...
import {
  POAP,
  FinishedWithError,
  MintCodeNotFoundError,
  MintCodeAlreadyUsedError,
  MintCodeExpiredError,
  AddressAlreadyMintDropError,
...
try {
  ...
  } else if (error instanceof MintCodeNotFoundError) {
    console.error('The mint code does not exists', error.message);
    // Action: Notify the user that the mint code is non existant.
  } else if (error instanceof MintCodeAlreadyUsedError) {
  ...
  } else if (error instanceof MintCodeExpiredError) {
    console.error('This mint code has expired', error.message);
    // Action: Notify the user that the mint code has expired.
  } else if (error instanceof AddressAlreadyMintDropError) {
    console.error('This address already owns a POAP for the same drop');
    // Action: Can lookup the POAP owned and show that one instead.
  } else {
